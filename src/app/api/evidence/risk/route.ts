...
// POST /api/evidence/risk — link a file to a risk item.
export async function POST(req: Request) {
  try {
    ...
    const link = await linkEvidence("risk", {
      companyId: ctx.companyId,
      userId: ctx.user.id,
      targetId: parsed.data.riskItemId,
    ...
    if (status === 500) console.error("[api/evidence/risk POST]", err);
